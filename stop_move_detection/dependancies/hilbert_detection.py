...
from dependancies.utils import *
from dependancies.db_connection import get_gps_hilbert, get_gps_hilbert_vgp
...
def stop_hilbert(participant_virtual_id, density_threshold=100, hilbert_power=3, merge_min=2):
    ...
    df['stop'] = -1
    df_count = df.groupby('hilbert').count().sort_values(
        'participant_virtual_id',  ascending=False)
    ...
    for item in df_count.index:
        if np.ceil(item/4**hilbert_power) in np.ceil(df[df.stop != -1].hilbert.unique()/4**hilbert_power):
            if len(list(set(df[(np.ceil(df.hilbert/4**hilbert_power) == np.ceil(item/4**hilbert_power)) & (df.stop != -1)].stop))) == 1:
                df.loc[df.hilbert == item, 'stop'] = list(set(df[(np.ceil(
                    df.hilbert/4**hilbert_power) == np.ceil(item/4**hilbert_power)) & (df.stop != -1)].stop))[0]
            else:
    ...
    while index < len(list_df)-1:
        if list_df[index].time.max() - list_df[index].time.min() < datetime.timedelta(minutes=merge_min):
            list_df[index-1] = pd.concat([list_df[index-1],
