...
from dependancies.db_connection import get_gps, get_gps_2
...
def stop_skmob(participant_virtual_id=987014104, stop_radius_factor=0.2,
    ...
    cstdf = clustering.cluster(
        stdf, cluster_radius_km=cluster_radius_km, min_samples=1)
    ...
    for i in range(lendf):
        ...
        df.loc[(df.time >= cstdf.datetime.iloc[i]) & (
            df.time <= cstdf.leaving_datetime.iloc[i]), 'stops'] = cstdf.cluster.iloc[i]
