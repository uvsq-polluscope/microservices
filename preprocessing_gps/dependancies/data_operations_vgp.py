...
#             abs(res['activity_time'].iloc[i] - res[res.drop(['activity_time', 'activity', 'event', 'verification'], axis=1).columns].iloc[i]).min()
...
def split_dataframe(df,columnName="BC",time=3600,show=False):
    ...
    for i in range(1,len(df)-1):
        #if df.time_id.iloc[i+1]-df.time_id.iloc[i]>10: #
...
def peaks_detection_(df,columnName,threshold,show=False):
    indices=[]
    for i in range(1,len(df[columnName])-1):
        if abs(df[columnName].iloc[i+1]-df[columnName].iloc[i])>=threshold and (df["time"].iloc[i+1]-df["time"].iloc[i]).seconds==60 and df["activity"].iloc[i+1]==df["activity"].iloc[i]        and df["event"].iloc[i+1]==df["event"].iloc[i]:
#             indices.append(i)
            if df[columnName].iloc[i]<0:
                indices.append(i)
            else:
                indices.append(i+1)
    if show==True:
...
def mean_peaks_detection_1(df_,columnName,window_size=10,show=False):
    indices=[]
    avg_map={}
    dfs=split_dataframe(df_,time=360,show=False)    
    th=df_[columnName].quantile(0.96)
    for df in dfs:        
        if len(df)<window_size:
            ...
            if not temp.empty:
                ...
                for i in range(len(df[columnName])):
                    if abs(abs(df[columnName].iloc[i])-abs(temp['columnName_Mean'].values[0]))>=threshold and abs(df[columnName].iloc[i])>th:
                            indices.append(df[columnName].index[i])
            ...
            for i in range(len(df[columnName])):
                ...
                if i<=margin:
                    threshold=(3/2)*abs(df['columnName_Mean'].iloc[i+2])
                    if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i+margin]))>=threshold and abs(df[columnName].iloc[i])>th:
                        indices.append(df[columnName].index[i])
                    ...
                    if i<=df.index[-margin]:                
                        ...
                        threshold=(3/2)*abs(df['columnName_Mean'].iloc[i])
                        if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i]))>=threshold and abs(df[columnName].iloc[i])>th:                                                        
                            indices.append(df[columnName].index[i])
...
def mean_peaks_detection_negative_replaced_by_abs_1(df__,columnName,window_size=10,show=False):
    indices=[]
    avg_map={}
    df_=df__.copy()
    th=df_[columnName].quantile(0.97)
    ...
    for df in dfs:
        if len(df)<window_size:
            ...
            for i in range(len(df[columnName])):
                if abs(abs(df[columnName].iloc[i])-abs(temp['columnName_Mean'].values[0]))>=threshold and df[columnName].iloc[i]>th:
                        indices.append(df[columnName].index[i])
                        avg_map[df[columnName].index[i]]=temp['columnName_Mean'].values[0]
        else:
            df['columnName_Mean'] = df[columnName].rolling(window=window_size, center=True ).mean()
            for i in range(len(df[columnName])):
                margin=int(window_size/2)
                if i<=window_size/2:
                    threshold=(3/2)*abs(df['columnName_Mean'].iloc[i+margin])
                    threshold=(3)*threshold
                    if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i+margin]))>=threshold and df[columnName].iloc[i]>th:
                        indices.append(df[columnName].index[i])
                        avg_map[df[columnName].index[i]]=df['columnName_Mean'].iloc[i+margin]
                else:
                    if i<=df.index[-margin]:                
                        ...
                        threshold=(3/2)*abs(df['columnName_Mean'].iloc[i])
                        threshold=(3)*threshold
                        if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i]))>=threshold and df[columnName].iloc[i]>th:                                                        
                            indices.append(df[columnName].index[i])
                            avg_map[df[columnName].index[i]]=df['columnName_Mean'].iloc[i]
    if show==True:
        plotly.offline.init_notebook_mode(connected=True)
...
def mean_peaks_replacing_negative_replaced_by_zero(df,columnName,window_size=10,show=False):
    peaks=mean_peaks_detection_negative_replaced_by_zero(df,columnName,window_size,show)
    indices=peaks[0]
    avg_map=peaks[1]
    df_new=df.copy()
    df_new["Replaced "+columnName]=np.zeros(len(df_new))
...
def mean_peaks_detection_negative_replaced_by_zero_1(df__,columnName,window_size=10,show=False):
    indices=[]
    avg_map={}
    df_=df__.copy()
    for i in range(len(df_)):
        if df_[columnName].iloc[i]<0:
            df_[columnName].replace(df_[columnName].iloc[i],0,inplace=True)
    dfs=split_dataframe(df_,time=360,show=False)
    for df in dfs:
        if len(df)<window_size:
            df['columnName_Mean'] = df[columnName].rolling(window=len(df), center=True ).mean()
            ...
            for i in range(len(df[columnName])):
                if i==0 or i==1:
                    ...
                    if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i+2]))>=threshold:
                        indices.append(df[columnName].index[i])
                        avg_map[df[columnName].index[i]]=df['columnName_Mean'].iloc[i+2]
                else:
                    if i==df.index[-1] or i==df.index[-2]:                
                        threshold=(3/2)*abs(df['columnName_Mean'].iloc[i-2])
                        if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i-2]))>=threshold:
...
def mean_peaks_removing_negative_replaced_by_abs_1(df,columnName,window_size=10,show=False):
    indices=mean_peaks_detection_negative_replaced_by_abs(df,columnName,window_size,show)[0]
    df_new=df.copy()
...
def mean_peaks_detection_after_negative_removal(df_,columnName,window_size=10,show=False,time=360):
    indices=[]
    avg_map={}
    dfs=split_dataframe(df_,time=time,show=False)
    for df in dfs:
        if len(df)<window_size:
            df['columnName_Mean'] = df[columnName].rolling(window=len(df), center=True ).mean()
            temp=df.dropna(subset=["columnName_Mean"])
            ...
            for i in range(len(df[columnName])):
                if i==0 or i==1:
                    threshold=2*abs(df['columnName_Mean'].iloc[i+2])
                    if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i+2]))>=threshold:
                        indices.append(df[columnName].index[i])
                        avg_map[df[columnName].index[i]]=df['columnName_Mean'].iloc[i+2]
                else:
                    if i==df.index[-1] or i==df.index[-2]:                
                        ...
                        threshold=2*abs(df['columnName_Mean'].iloc[i])
                        if abs(abs(df[columnName].iloc[i])-abs(df['columnName_Mean'].iloc[i]))>=threshold:                                                        
                            indices.append(df[columnName].index[i])
                            avg_map[df[columnName].index[i]]=df['columnName_Mean'].iloc[i]
    if show==True:
...
def spikes_detection_validation_with_activities_negative_replaced_by_zero(df_,columnName,approach=3,threshold=None,show_all=False,show_plot=False):
    aggregation=[]
    new_df=calculate_avtivity_time(df_)
    avg_map={}
    if approach==3:
        indices,avg_map=mean_peaks_detection_negative_replaced_by_zero(df_,columnName,show=False)        
    else:
        if approach==2:
            if threshold==None:
                indices=peaks_detection__(df_,columnName,show=False)
...
def spikes_detection_validation_with_activities_negative_replaced_by_abs(df_,columnName,approach=3,threshold=None,show_all=False,show_plot=False,time=360):
    aggregation=[]
    new_df=calculate_avtivity_time(df_)
    avg_map={}
    if approach==3:
        indices,avg_map=mean_peaks_detection_negative_replaced_by_abs(df_,columnName,show=False)
    else:
        if approach==2:
            if threshold==None:
                indices=peaks_detection__(df_,columnName,show=False)
...
def spikes_detection_validation_with_changes_negative_replaced_by_zero(df_,columnNames,columnName,approach=3,window_size=10,threshold=None,show_all=False,show_plot=False,time=360):
    aggregation=[]
    avg_map={}
    values=['activity_time', 'Humidity', 'Temperature']
    df_temperature, newdf_t, prec_t, rec_t = cusum_(df_, 'Temperature', [0.6], [0.05], False)
    ...
    if approach==3:
        indices,avg_map=mean_peaks_detection_negative_replaced_by_zero(df_,columnName,window_size,show=False)
    else:
        if approach==2:
            if threshold==None:
                indices=peaks_detection__(df_,columnName,show=False)
...
# def spikes_detection_validation_with_changes_negative_replaced_by_abs(df_,columnNames,columnName,approach=3,window_size=10,threshold=None,show_all=False,show_plot=False,time=360):
#     aggregation=[]
#     avg_map={}
#     df_temperature, newdf_t, prec_t, rec_t = cusum_(df_, 'Temperature', [0.6], [0.05], False)
#     df_humidity, newdf_h, prec_h, rec_h = cusum_(df_, 'Humidity', [4], [0.05], False)
...
#     res2=compute_verification_column(TH3,['activity_time', 'Humidity', 'Temperature', 'NO2', 'BC'])
#     res2.drop(res2[res2.activity_time == df_.sort_values(['time']).time.iloc[0]].index, inplace = True)
#     res2.drop_duplicates('verification', inplace = True)
#     new_df=get_concentration_along_changes(df_,res2,columnNames).index.to_list()
    
#     if approach==3:
#         indices,avg_map=mean_peaks_detection_negative_replaced_by_abs(df_,columnName,window_size,show=False)
#     else:
#         if approach==2:
#             if threshold==None:
#                 indices=peaks_detection__(df_,columnName,show=False)
...
def spikes_detection_along_range_changes_negative_replaced_by_abs_1(df_,columnNames,columnName,approach=3,threshold=None,show_all=False,show_plot=False):
    aggregation=[]
    avg_map={}
    df_temperature, newdf_t, prec_t, rec_t = cusum_(df_, 'Temperature', [0.6], [0.05], False)
    df_humidity, newdf_h, prec_h, rec_h = cusum_(df_, 'Humidity', [4], [0.05], False)
    ...
    if approach==3:
        indices,avg_map=mean_peaks_detection_negative_replaced_by_abs(new_df,columnName,show=False)
    else:
        if approach==2:
            if threshold==None:
                indices=peaks_detection__(df_,columnName,show=False)
...
def spikes_detection_along_range_changes_negative_replaced_by_zero(df_,columnNames,columnName,approach=3,threshold=None,show_all=False,show_plot=False):
    aggregation=[]
    avg_map={}
    values=['activity_time', 'Humidity', 'Temperature']
    df_temperature, newdf_t, prec_t, rec_t = cusum_(df_, 'Temperature', [0.6], [0.05], False)
    ...
    if approach==3:
        indices,avg_map=mean_peaks_detection_negative_replaced_by_zero(new_df,columnName,show=False)
    else:
        if approach==2:
            if threshold==None:
                indices=peaks_detection__(df_,columnName,show=False)
...
def spikes_detection_positive_BC(df_,columnNames,columnName,approach=3,threshold=None,show_all=False,show_plot=False):
    aggregation=[]
    avg_map={}
    values=['activity_time', 'Humidity', 'Temperature']
    df_temperature, newdf_t, prec_t, rec_t = cusum_(df_, 'Temperature', [0.6], [0.05], False)
...
def mean_peaks_detection_negative_replaced_by_abs_2(df__,columnName,window_size=10,show=False):
    indices=[]
    avg_map={}
    df_=df__.copy()
    th=df_[columnName].quantile(0.97)
    th2=df_[columnName].quantile(0.9)
    for i in range(len(df_)):
        if df_[columnName].iloc[i]<0:
            df_[columnName].replace(df_[columnName].iloc[i],abs(df_[columnName].iloc[i]),inplace=True)
    dfs=split_dataframe(df_,time=360,show=False)
...
def mean_peaks_removing_negative_replaced_by_abs(df,columnName,min_threshold,max_threshold,window_size=10,show=False):
    indices,avg,detected,indices2=mean_peaks_detection_negative_replaced_by_abs(df,columnName,min_threshold=min_threshold,max_threshold=max_threshold,window_size=window_size,show=show)
    df_new=df.copy()
    df_new["Replaced"]=np.zeros(len(df_new))
    for key in indices:    
        df_new["Replaced"].iloc[key]=1
        df_new.replace(df_new.iloc[key][columnName],np.nan,inplace=True)        
    return [df_new,detected,indices,indices2]


# In[63]:


def mean_peaks_removing_all_peaks_negative_replaced_by_abs_1(df_,columnName,min_threshold,max_threshold,window_size=10,show=False):
    df=df_.copy()
    df["original_index"]=df.index
    indices=[]
    indices2=[]    
    df_new,detected,index,index2=mean_peaks_removing_negative_replaced_by_abs(df,columnName,min_threshold=min_threshold,max_threshold=max_threshold,window_size=window_size,show=False)
    indices+=index
    indices2+=index2
    while detected>3:
        s=df_new.dropna(subset=[columnName])
        s.reset_index(drop=True,inplace=True)
        df_new,detected,index,index2=mean_peaks_removing_negative_replaced_by_abs(s,columnName,min_threshold=min_threshold,max_threshold=max_threshold,window_size=window_size,show=False)
        indices+=index
        indices2+=index2
    ...
    if show==True:
        ...
        fig.add_trace(go.Scatter(
            x=indices,
            y=[df[columnName][j] for j in indices2],
            mode='markers',
            marker=dict(
            size=8,
            color='red',
...
def spikes_detection_along_range_changes_negative_replaced_by_abs(df_,columnNames,columnName,min_threshold,max_threshold,approach=3,threshold=None,show_all=False,show_plot=False):
    aggregation=[]
    avg_map={}
    values=['activity_time', 'Humidity', 'Temperature']
    df_["original_index"]=df_.index
    ...
    if approach==3:
#         indices,avg_map,detected,indices2=mean_peaks_detection_negative_replaced_by_abs(new_df,columnName,show=False)
        indices=[]
        indices2=[]    
        df_new,detected,index,index2=mean_peaks_removing_negative_replaced_by_abs(df_,columnName,min_threshold=min_threshold,max_threshold=max_threshold,window_size=window_size,show=False)
        indices+=index
        indices2+=index2
        while detected>3:
            s=df_new.dropna(subset=[columnName])
        ...
        if approach==2:
            ...
            if approach==1:
                if threshold==None:
                    ...
                    indices=peaks_detection_(df_,columnName,threshold=threshold,show=False)
    if show_all==True:
        show_all_(df_,columnName,indices2,df_change_index)
    else:
        if show_plot ==True:
            show_plot_(df_,columnName,indices2,df_change_index)
    for k in df_change_index:
        for m in indices2:
            if k==m:
                aggregation.append(k)
...
def spikes_detection_validation_with_changes_negative_replaced_by_abs(df_,columnName,min_threshold,max_threshold,q=0.9999,interval=2,approach=3,window_size=10,threshold=None,show_all=False,show_plot=False,time=360):
    aggregation=[]
...
#############order
    ...
    return [indices,list(set(aggregation)),avg_map]


# In[67]:


def mean_peaks_detection_negative_replaced_by_Zero(df__,columnName,min_threshold=-2000,max_threshold=50000,window_size=10,show=False):
    indices=[]
    indices2=[]
    avg_map={}
    detected=0
    ...
    #             threshold=(5)*threshold
                for i in range(len(df[columnName])):
                    if (((df[columnName].iloc[i]>th and df__[columnName].iloc[i]>0) or (df[columnName].iloc[i]<th and df__[columnName].iloc[i]<0) ) and abs(df__[columnName].iloc[df[columnName].index[i]]-df__[columnName].shift(1).iloc[df[columnName].index[i]])>=th2 and abs(df__[columnName].iloc[df[columnName].index[i]]-df__[columnName].shift(-1).iloc[df[columnName].index[i]])>=th2 and ((df__[columnName].iloc[df[columnName].index[i]]>df__[columnName].shift(1).iloc[df[columnName].index[i]] and df__[columnName].iloc[df[columnName].index[i]]>df__[columnName].shift(-1).iloc[df[columnName].index[i]] and df__[columnName].iloc[df[columnName].index[i]]>0) or (df__[columnName].iloc[df[columnName].index[i]]<df__[columnName].shift(1).iloc[df[columnName].index[i]] and df__[columnName].iloc[df[columnName].index[i]]<df__[columnName].shift(-1).iloc[df[columnName].index[i]] and df__[columnName].iloc[df[columnName].index[i]]<0)))  or (df__[columnName].iloc[df[columnName].index[i]]<min_threshold and df__[columnName].iloc[df[columnName].index[i]]<0) or (df__[columnName].iloc[df[columnName].index[i]]>max_threshold and df__[columnName].iloc[df[columnName].index[i]]>0):
                        indices2.append(int(df["original_index"].iloc[i]))
                        avg_map[int(df["original_index"].iloc[i])]=temp['columnName_Mean'].values[0]
                        indices.append(df[columnName].index[i])
    #                     avg_map[df[columnName].index[i]]=temp['columnName_Mean'].values[0]
